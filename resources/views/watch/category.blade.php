...
@section('title', ($category?->name ?? 'Search').' - RifiMedia Watch')
...
@section('content')
    <section class="iptv-shell">
        <aside class="iptv-sidebar">
            <x-logo />
            <nav>
                <a href="{{ route('watch.index') }}">Home</a>
        ...
        <div class="iptv-main">
            <header class="iptv-page-head">
                <div>
                    <p class="rm-eyebrow">{{ $category?->type ? str($category->type)->headline() : 'Search' }}</p>
                    <h1>{{ $category?->name ?? 'Search your IPTV library' }}</h1>
                </div>
                <form action="{{ $category ? route('watch.category', $category) : route('watch.search') }}" class="iptv-search">
                    <input name="q" value="{{ request('q') }}" placeholder="Search this view">
            ...
            @if($items->isEmpty())
                <div class="empty-state">
                    ...
                    <p>Try another category or import a playlist with matching content.</p>
                </div>
            @else
