        ...
        <main class="rm-watch-main">
            <header class="rm-player-header rm-player-header--hero">
                ...
                <div class="rm-player-header__actions">
                    <a class="rm-btn rm-btn-secondary rm-btn-sm" href="{{ route('live-tv') }}">{{ __("Back to Live TV") }}</a>
                    <a class="rm-btn rm-btn-primary rm-btn-sm" href="{{ route('sports.football') }}">{{ __("Football Scores") }}</a>
                </div>
            </header>
            ...
            <section class="rm-glass-card rm-match-overview">
                <div>
                    <span class="rm-live-badge rm-live-badge--small"><i></i> {{ __("Now watching") }}</span>
                    ...
                    @else
                        <p>{{ __("Live broadcast - program guide updates will appear here when published.") }}</p>
                    @endif
                </div>
                <div class="rm-server-card">
                    <span>{{ __("Stream quality") }}</span>
                    <strong>{{ __("Auto HD") }}</strong>
                    <small>{{ trans_choice('common.servers_available', count($sources), ['count' => count($sources)]) }}</small>
                </div>
            </section>
            ...
            <section class="rm-section rm-section--flush">
                <div class="rm-section-header">
                    <div>
                        <p class="rm-eyebrow">{{ __("Electronic program guide") }}</p>
                        <h2>{{ __("Up next") }}</h2>
                    </div>
                </div>
                        ...
                        @foreach($programs as $program)
                            <article class="{{ $program->start_time <= now() && $program->end_time > now() ? 'is-now' : '' }}">
                                ...
                                <div>
                                    ...
                                    <p>{{ $program->description ?: __('Live programming from this channel.') }}</p>
                                </div>
                            </article>
                        @endforeach
                    </div>
                @else
                    <div class="rm-empty-state">
                        <span>{{ __("No schedule yet") }}</span>
                        <strong>{{ __("This channel is live, but no program guide has been published.") }}</strong>
                    </div>
                @endif
            </section>
        </main>
