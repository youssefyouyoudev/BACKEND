    ...
    <section class="rm-section">
        @if($items->isEmpty())
            <x-empty-state
                ...
                message="{{ __("Browse live scores, channels, and league coverage while this directory is prepared.") }}"
                action="{{ __("View live scores") }}"
                :href="route('sports.football')"
                secondary-action="{{ __("Browse channels") }}"
            ...
            <div class="rm-directory-grid">
                @foreach($items as $item)
                    <article class="rm-directory-card">
                        ...
                        <h2>{{ $item['name'] }}</h2>
                        <p>{{ __("Fixtures, standings, news, match previews, and related coverage in one place.") }}</p>
                        <a href="{{ $kind === 'leagues' ? route('leagues.show', $item['slug']) : route('teams.show', $item['slug']) }}">{{ __("Open page") }}</a>
                    </article>
                @endforeach
            </div>
        @endif
