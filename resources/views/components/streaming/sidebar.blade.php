...
<aside class="stream-sidebar" aria-label="{{ __("Streaming navigation") }}">
    <div class="stream-sidebar__top">
        <a href="{{ route('home') }}" class="stream-logo" aria-label="{{ __("RiFi Media TV home") }}">
        ...
        <button type="button" class="stream-sidebar__menu" aria-label="{{ __("Collapse menu") }}">
            <svg viewBox="0 0 24 24" fill="none"><path d="M4 7h16M4 12h16M4 17h16"/></svg>
        </button>
    </div>

    <nav class="stream-nav" aria-label="{{ __("Main menu") }}">
        ...
        <a href="{{ route('live-tv') }}" class="{{ $active === 'live' ? 'is-active' : '' }}">
            ...
            <span>{{ __("My Channels") }}</span>
        </a>
    </nav>

    <section class="recommended-list" aria-label="{{ __("Recommended channels") }}">
        <h2>{{ __("Recommended Channels") }}</h2>
        <div class="recommended-list__items">
            ...
            @empty
                <p class="recommended-list__empty">{{ __("Import and approve playlists to populate recommendations.") }}</p>
            @endforelse
        </div>
    </section>
</aside>
