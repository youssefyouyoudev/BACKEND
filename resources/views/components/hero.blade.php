...
@php
    ...
    $category = data_get($channel, 'category') ?: data_get($channel, 'group_title') ?: __('Premium sports TV');
    $program = data_get($channel, 'program.title') ?: __('Live matches, channels, and featured broadcasts');
...
<section class="rm-hero" aria-labelledby="rm-hero-title">
    ...
    <div class="rm-hero__content">
        <span class="rm-live-badge"><i></i> {{ __("Live now") }}</span>
        <p class="rm-eyebrow">{{ __("Featured broadcast") }}</p>
        ...
        <div class="rm-hero__actions">
            <a href="{{ $watchUrl }}" class="rm-btn rm-btn-primary">{{ __("Watch Live") }}</a>
            <a href="#channels" class="rm-btn rm-btn-secondary">{{ __("Browse Matches") }}</a>
        </div>
    </div>

    <aside class="rm-hero-card" aria-label="{{ __("Featured channel preview") }}">
