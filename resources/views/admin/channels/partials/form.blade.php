<div class="form-grid">
    <div class="field">
        ...
        <input id="name" name="name" value="{{ old('name', $channel->name ?? '') }}" required maxlength="140" placeholder="{{ __("RiFi Sports 1") }}">
    </div>
    <div class="field">
        <label for="slug">{{ __("Slug") }}</label>
        <input id="slug" name="slug" value="{{ old('slug', $channel->slug ?? '') }}" maxlength="160" placeholder="{{ __("rifi-sports-1") }}">
    </div>
    <div class="field">
        <label for="playlist_id">{{ __("Playlist") }}</label>
    ...
    <div class="field">
        ...
        <input id="country" name="country" value="{{ old('country', $channel->country ?? '') }}" maxlength="120" placeholder="{{ __("Morocco") }}">
    </div>
    <div class="field">
        <label for="stream_type">{{ __("Stream type") }}</label>
    ...
    <div class="field form-grid__wide">
        <label for="stream_url">{{ __("Primary stream URL") }}</label>
    ...
    <div class="field form-grid__wide">
        <label for="aliases">{{ __("TheSportsDB aliases") }}</label>
        <textarea id="aliases" name="aliases" rows="4" placeholder="{{ __("beIN Sports HD 1&#10;beIN Sports MENA 1") }}">{{ old('aliases', isset($channel) ? implode("\n", $channel->aliases ?? []) : '') }}</textarea>
        <small class="field__hint">{{ __("Add one channel alias per line, or separate aliases with commas.") }}</small>
    </div>
    <div class="field">
        <label for="featured_rank">{{ __("Featured rank") }}</label>
