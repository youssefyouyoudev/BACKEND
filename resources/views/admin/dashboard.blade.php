...
@section('content')
    <section class="page-header">
        <div>
            <p class="page-header__eyebrow">{{ __("Admin Dashboard") }}</p>
            <h1>{{ __("Playlist ingestion and channel publishing.") }}</h1>
            <p class="page-header__copy">
                {{ __("Import legal M3U sources once, parse them asynchronously, and expose a fast channel browser to the public player.") }}
            </p>
        </div>
        <div class="page-header__status-card">
            <span class="page-header__status-label">{{ __("Last successful sync") }}</span>
            <strong>{{ $lastSync ? $lastSync->diffForHumans() : __('No sync yet') }}</strong>
            <span class="page-header__status-note">{{ $lastSync?->format('M d, Y H:i') ?? __('Waiting for the first import.') }}</span>
        </div>
    </section>

    <section class="stats-grid">
        <article class="stat-card">
            <span class="stat-card__label">{{ __("Playlists") }}</span>
            <strong>{{ number_format($stats['playlists']) }}</strong>
        </article>
        <article class="stat-card">
            <span class="stat-card__label">{{ __("Channels") }}</span>
        ...
        <article class="stat-card">
            <span class="stat-card__label">{{ __("Queue mode") }}</span>
            <strong>{{ strtoupper(config('queue.default')) }}</strong>
        </article>
        <article class="stat-card">
            <span class="stat-card__label">{{ __("Online streams") }}</span>
            <strong>{{ number_format($stats['online_streams']) }}</strong>
        </article>
        <article class="stat-card">
            <span class="stat-card__label">{{ __("Offline streams") }}</span>
            <strong>{{ number_format($stats['offline_streams']) }}</strong>
        </article>
        <article class="stat-card">
            <span class="stat-card__label">{{ __("Unknown streams") }}</span>
            <strong>{{ number_format($stats['unknown_streams']) }}</strong>
        </article>
        <article class="stat-card">
            <span class="stat-card__label">{{ __("World Cup matches") }}</span>
        ...
        <article class="stat-card">
            <span class="stat-card__label">{{ __("Missing channel") }}</span>
            <strong>{{ number_format($stats['world_cup_missing_channel']) }}</strong>
        </article>
        <article class="stat-card">
            <span class="stat-card__label">{{ __("Missing commentator") }}</span>
    ...
    <section class="surface-card">
        <div class="surface-card__header">
            <div>
                <p class="surface-card__eyebrow">{{ __("World Cup 2026") }}</p>
                <h2>{{ __("Next group-stage matches") }}</h2>
            </div>
            <a class="button button--primary" href="{{ route('admin.world-cup-matches.index') }}">{{ __("Manage matches") }}</a>
        </div>
        <div class="wc-dashboard-list">
            ...
            @empty
                <p>{{ __("No upcoming matches are currently stored.") }}</p>
            @endforelse
        </div>
    </section>
    ...
    <section class="admin-grid">
        <article class="surface-card" id="playlist-form">
            <div class="surface-card__header">
                <div>
                    <p class="surface-card__eyebrow">{{ __("Add new source") }}</p>
                    <h2>{{ __("Register an M3U playlist") }}</h2>
                </div>
                <span class="surface-card__badge">{{ __("Safe URL validation enabled") }}</span>
            </div>
            ...
            <form method="POST" action="{{ route('admin.playlists.store') }}" class="form-card form-card--embedded" enctype="multipart/form-data">
                ...
                <div class="field">
                    ...
                    <select id="input_type" name="input_type" required data-playlist-input-type>
                        <option value="{{ \App\Models\Playlist::INPUT_TYPE_M3U_URL }}" @selected($selectedInputType === \App\Models\Playlist::INPUT_TYPE_M3U_URL)>{{ __("M3U URL") }}</option>
                        <option value="{{ \App\Models\Playlist::INPUT_TYPE_XTREAM }}" @selected($selectedInputType === \App\Models\Playlist::INPUT_TYPE_XTREAM)>{{ __("Xtream Codes") }}</option>
                        <option value="{{ \App\Models\Playlist::INPUT_TYPE_ACTIVE_CODE }}" @selected($selectedInputType === \App\Models\Playlist::INPUT_TYPE_ACTIVE_CODE)>{{ __("Active Code") }}</option>
                        <option value="{{ \App\Models\Playlist::INPUT_TYPE_UPLOAD }}" @selected($selectedInputType === \App\Models\Playlist::INPUT_TYPE_UPLOAD)>{{ __("Upload M3U File") }}</option>
                    </select>
                </div>

                <div class="field">
                    <label for="name">{{ __("Playlist name") }}</label>
                    <input id="name" type="text" name="name" value="{{ old('name') }}" required maxlength="120" placeholder="{{ __("Premium Sports Feed") }}">
                </div>
                ...
                <div class="field" data-playlist-field="xtream">
                    ...
                    <select id="output" name="output">
                        <option value="mpegts" @selected(old('output', 'mpegts') === 'mpegts')>{{ __("mpegts") }}</option>
                        <option value="hls" @selected(old('output') === 'hls')>{{ __("hls") }}</option>
                    </select>
                ...
                <div class="legal-callout">
                    <strong>{{ __("Legal reminder") }}</strong>
                    <p>{{ $appSettings['legal_notice'] }}</p>
                </div>

                <button type="submit" class="button button--primary" id="save-playlist-btn">{{ __("Save &amp; Import") }}</button>
            </form>
        </article>
        ...
        <article class="surface-card surface-card--accent">
            <div class="surface-card__header">
                <div>
                    <p class="surface-card__eyebrow">{{ __("Operations") }}</p>
                    <h2>{{ __("How parsing works") }}</h2>
                </div>
            </div>
            <ol class="check-list">
                <li>{{ __("Admin adds a legal playlist URL or uploads an M3U file.") }}</li>
                <li>{{ __("The parse action runs immediately in `sync` mode or queues a background job.") }}</li>
                <li>{{ __("The parser extracts channel names, logos, categories, and stream URLs.") }}</li>
                <li>{{ __("Old channels are cleared and the playlist sync timestamp is refreshed after every import.") }}</li>
            </ol>
        </article>
    </section>
    ...
    <section class="surface-card">
        <div class="surface-card__header">
            <div>
                <p class="surface-card__eyebrow">{{ __("Stream health") }}</p>
                <h2>{{ __("Failed sources") }}</h2>
            </div>
            <span class="surface-card__badge">{{ __("Checked by scheduler") }}</span>
        </div>

        @if($failedSources->isEmpty())
            <div class="empty-state empty-state--compact">
                <h3>{{ __("No failed stream sources recorded.") }}</h3>
                <p>{{ __("Run") }} <code>{{ __("php artisan streams:check-health") }}</code> {{ __("to populate health status.") }}</p>
            </div>
        @else
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        @foreach($failedSources as $source)
                            <tr>
                                <td>
                                    <strong>{{ $source->channel?->clean_display_name ?? __('Unknown channel') }}</strong>
                                    <span class="table-subtle">{{ $source->label ?: __('Server :number', ['number' => $source->priority]) }}</span>
                                </td>
                                ...
                                <td>{{ $source->last_checked_at?->diffForHumans() ?? __('Never') }}</td>
                                <td><span class="table-url">{{ $source->last_error ?: __('No details') }}</span></td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
    ...
    <section class="surface-card" id="playlist-table">
        <div class="surface-card__header">
            <div>
                <p class="surface-card__eyebrow">{{ __("Stored catalogs") }}</p>
                <h2>{{ __("Playlists and parsing status") }}</h2>
            </div>
        </div>
        ...
        @if ($playlists->count() === 0)
            <div class="empty-state">
                ...
                <p>{{ __("Add your first M3U source above to start building the channel library.") }}</p>
            </div>
        @else
            <div class="table-shell">
                <table class="data-table">
                            ...
                            <tr>
                                ...
                                <td>
                                    @if ($playlist->input_type === \App\Models\Playlist::INPUT_TYPE_UPLOAD_FILE)
                                        <span class="table-subtle">{{ __("Uploaded file") }}</span>
                                    ...
                                    @elseif ($playlist->input_type === \App\Models\Playlist::INPUT_TYPE_ACTIVE_CODE)
                                        <span class="table-subtle">{{ __("Active Code") }}</span>
                                        <span class="table-url">{{ $playlist->masked_m3u_url ?: __('M3U URL needed before parsing') }}</span>
                                    @else
                                        <span class="table-subtle">{{ __("Remote URL") }}</span>
                                ...
                                <td>
                                    <strong>{{ $playlist->last_synced_at?->diffForHumans() ?? __('Never') }}</strong>
                                    <span class="table-subtle">{{ $playlist->last_synced_at?->format('M d, Y H:i') ?? __('Not parsed yet') }}</span>
                                </td>
                                <td class="text-end">
                                    <form method="POST" action="{{ route('admin.playlists.parse', $playlist) }}">
                                        ...
                                        >
                                            {{ in_array($playlist->status, ['queued', 'processing'], true) ? __('Parsing…') : __('Re-parse') }}
                                        </button>
                                    </form>

                                    <details class="playlist-editor">
                                        <summary class="button button--ghost">{{ __("Edit") }}</summary>
                                        <form method="POST" action="{{ route('admin.playlists.update', $playlist) }}" class="playlist-editor__form" enctype="multipart/form-data">
                                            ...
                                            <div class="field">
                                                <label for="playlist-{{ $playlist->id }}-input-type">{{ __("Playlist Input Type") }}</label>
                                                <select id="playlist-{{ $playlist->id }}-input-type" name="input_type" required data-playlist-input-type>
                                                    <option value="{{ \App\Models\Playlist::INPUT_TYPE_M3U_URL }}" @selected($editInputType === \App\Models\Playlist::INPUT_TYPE_M3U_URL)>{{ __("M3U URL") }}</option>
                                                    <option value="{{ \App\Models\Playlist::INPUT_TYPE_XTREAM }}" @selected($editInputType === \App\Models\Playlist::INPUT_TYPE_XTREAM)>{{ __("Xtream Codes") }}</option>
                                                    <option value="{{ \App\Models\Playlist::INPUT_TYPE_ACTIVE_CODE }}" @selected($editInputType === \App\Models\Playlist::INPUT_TYPE_ACTIVE_CODE)>{{ __("Active Code") }}</option>
                                                    <option value="{{ \App\Models\Playlist::INPUT_TYPE_UPLOAD }}" @selected($editInputType === \App\Models\Playlist::INPUT_TYPE_UPLOAD)>{{ __("Upload M3U File") }}</option>
                                                </select>
                                            </div>

                                            <div class="field">
                                                <label for="playlist-{{ $playlist->id }}-name">{{ __("Playlist name") }}</label>
                                                <input id="playlist-{{ $playlist->id }}-name" type="text" name="name" value="{{ old('name', $playlist->name) }}" required maxlength="120">
                                            </div>

                                            <div class="field" data-playlist-field="m3u_url active_code">
                                                <label for="playlist-{{ $playlist->id }}-url">{{ __("M3U URL") }}</label>
                                                <input
                                                ...
                                                >
                                                <small class="field__hint">{{ __("Remote mode requires a URL. Active Code mode can save without one.") }}</small>
                                            </div>

                                            <div class="field" data-playlist-field="upload">
                                                <label for="playlist-{{ $playlist->id }}-file">{{ __("Replace with file") }}</label>
                                                ...
                                                <small class="field__hint">{{ __("Saving changes runs a fresh parse immediately.") }}</small>
                                            </div>

                                            <div class="field" data-playlist-field="xtream">
                                                <label for="playlist-{{ $playlist->id }}-server-url">{{ __("Server URL") }}</label>
                                            ...
                                            <div class="field" data-playlist-field="xtream">
                                                <label for="playlist-{{ $playlist->id }}-username">{{ __("Username") }}</label>
                                                <input id="playlist-{{ $playlist->id }}-username" name="username" value="{{ old('username', $playlist->username) }}" autocomplete="off">
                                            </div>

                                            <div class="field" data-playlist-field="xtream">
                                                <label for="playlist-{{ $playlist->id }}-password">{{ __("Password") }}</label>
                                                <input id="playlist-{{ $playlist->id }}-password" type="password" name="password" autocomplete="new-password" placeholder="{{ $playlist->password ? __('Leave unchanged') : '' }}">
                                            </div>

                                            <div class="field" data-playlist-field="xtream">
                                                <label for="playlist-{{ $playlist->id }}-output">{{ __("Output") }}</label>
                                            ...
                                            <div class="field" data-playlist-field="active_code">
                                                <label for="playlist-{{ $playlist->id }}-active-code">{{ __("Active Code") }}</label>
                                                <input id="playlist-{{ $playlist->id }}-active-code" type="text" name="active_code" value="{{ old('active_code') }}" minlength="4" maxlength="64" pattern="[A-Za-z0-9]+" autocomplete="off" placeholder="{{ $playlist->active_code ? __('Leave unchanged') : '696966207988' }}">
                                                <small class="field__hint">{{ __("Use this when your provider gives you an activation code with or without an M3U link.") }}</small>
                                            </div>

                                            <button type="submit" class="button button--primary">{{ __("Save &amp; Re-parse") }}</button>
                                        </form>
                                    </details>

                                    <form method="POST" action="{{ route('admin.playlists.destroy', $playlist) }}" onsubmit="return confirm(@js(__('Delete this playlist and all imported channels?')));">
                                        @csrf
                                        @method('DELETE')
